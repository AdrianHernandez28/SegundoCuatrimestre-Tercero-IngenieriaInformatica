...
int main() {
  ...
  const long long lambda = calculateLambda(p, q);
  ...
  if (std::gcd(d, lambda) == 1) {
    std::cout << "PASO 2: Se comprueba que d es primo con lambda(n)=" << lambda << "..." << std::endl; 
  } else {
    std::cerr << "Error: d no es primo con lambda(n)." << std::endl;
    return -1;
  }
  const long long e = modularMultiplicativeInverse(d, lambda);
